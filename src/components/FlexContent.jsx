...
const FlexContent = ({ifExists,endpoint:{title,heading,text,url,img,btn}}) => {
  return (
   <>
   <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ? "flex-row-reverse" : "flex-row"}`}>
    <div className='max-w-lg lg:max-w-none w-full md:text-center grid items-center lg:justify-items-center'>
        ...
        <a href={url} className='flex items-center' target={"blank"} role='button' ><button className='button-theme bg-slate-900 shadow-slate-900 py-1.5 text-slate-100' type='button'>{btn}</button></a>
    </div>
